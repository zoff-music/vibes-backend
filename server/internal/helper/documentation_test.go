...
func TestPublicDocumentation(t *testing.T) {
	...
	tests := []documentationFilterTest{
		{name: "generated specification", document: generated, wantPaths: -1, wantDefinitions: -1},
		{name: "tagged operation and transitive cyclic schemas", document: `{"swagger":"2.0","paths":{"/public":{"get":{"responses":{"200":{"schema":{"$ref":"#/definitions/Public"}}}},"post":{"tags":["admin"]}},"/api/v2/admin/users":{"get":{}},"/only-admin":{"parameters":[],"get":{"tags":["admin"]}}},"definitions":{"Public":{"properties":{"child":{"$ref":"#/definitions/Child"}}},"Child":{"items":{"$ref":"#/definitions/Public"}},"Secret":{"type":"string"}},"tags":[{"name":"admin"}]}`, wantPaths: 1, wantDefinitions: 2},
		{name: "no definitions", document: `{"swagger":"2.0","paths":{"/public":{"get":{}}}}`, wantPaths: 1, wantDefinitions: 0},
		{name: "missing schema fails closed", document: `{"paths":{"/public":{"get":{"$ref":"#/definitions/Missing"}}}}`, wantError: true},
		{name: "malformed input fails closed", document: `{`, wantError: true},
	}
