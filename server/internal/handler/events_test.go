...
type roomEventsPublisherStub struct {
	...
	replay       *vibe.ReplaySubscription
}
...
func TestRoomEventsV2SendsCompactSnapshot(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			RoomEventsV2(publisher, &tt.storage, &tt.storage).ServeHTTP(response, request)
...
func TestRoomEventsV2UsesCompactMutation(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			RoomEventsV2(publisher, storage, storage).ServeHTTP(response, request)
