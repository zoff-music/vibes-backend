-- Add only_admin_add_songs column to room_settings
ALTER TABLE room_settings ADD COLUMN only_admin_add_songs INTEGER NOT NULL DEFAULT 0;
...
SELECT
    ...
    rs.allow_duplicates,
    rs.enabled_sources,
    rs.only_admin_add_songs
FROM rooms r
JOIN room_settings rs ON r.id = rs.room_id;

-- Update rooms_view_insert trigger
...
BEGIN
    ...
    VALUES (
        ...
        COALESCE(NEW.only_admin_add_songs, 0)
    );
END;

-- Update rooms_view_update trigger
...
BEGIN
    ...
    SET skip_allowed = NEW.skip_allowed,
        ...
        only_admin_add_songs = NEW.only_admin_add_songs
