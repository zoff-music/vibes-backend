...
-- Remove column (SQLite doesn't support DROP COLUMN in all versions, but we'll try)
-- Alternatively, recreate the table without the column.
-- For standard migration logic, we'll try:
ALTER TABLE room_settings DROP COLUMN enabled_sources;
