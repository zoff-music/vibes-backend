...
async function runBuild() {
  ...
  if (!result.success) {
    ...
    cssFilename =
      cssFiles.find((f) => f === 'index.css') ||
      cssFiles.find((f) => f.startsWith('client')) ||
      cssFiles[0] ||
      'index.css';

    // In production, hash the CSS file to prevent caching issues
    if (isProd && cssFilename === 'index.css') {
      ...
      if (existsSync(cssPath)) {
        ...
        const hash = crypto
          ...
          .digest('hex')
          .substring(0, 8);
        const hashedFilename = `index-${hash}.css`;
        const hashedPath = join('./dist', hashedFilename);

        fs.renameSync(cssPath, hashedPath);
        console.log(
          `[Build] Hashed CSS file: ${cssFilename} -> ${hashedFilename}`,
        );
        cssFilename = hashedFilename;
      }
    }
    ...
    if (cssFilename) {
      ...
      // cleanup unused client-*.css files generated by Bun from the import in client.tsx
      // We don't use these because they don't have the tailwind processing
      for (const file of cssFiles) {
        if (
          ...
          console.log(`[Build] Removing redundant CSS file: ${file}`);
          const fs = await import('node:fs');
          if (fs.existsSync(join('./dist', file))) {
