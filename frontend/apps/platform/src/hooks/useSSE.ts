...
const IN_FLIGHT_CONNECTIONS = new Map<
  string,
  Promise<any>
>();
  ...
  useEffect(() => {
    ...
    const setupConnection = async () => {
      ...
      if (!connection) {
        ...
        if (!isMounted) {
            console.log('[SSE] component unmounted during connection, parking connection', roomId);
            ...
            if (unsubscribe && !ACTIVE_CONNECTIONS.has(roomId)) {
                 ...
                 if (PENDING_CLEANUPS.has(roomId)) clearTimeout(PENDING_CLEANUPS.get(roomId));
                 const timeout = setTimeout(() => {
                    console.log('[SSE] executing cleanup for parked connection', roomId);
