...
class GoogleCastManager implements CastManager {
    ...
    return new Promise((resolve, reject) => {
      ...
      const checkInterval = setInterval(() => {
        ...
        if (window.chrome?.cast?.isAvailable) {
          ...
          clearInterval(checkInterval);
          const error = new Error('Google Cast API not available after timeout');
          this.notifyError({
            code: 'API_TIMEOUT',
            description: 'Google Cast API failed to initialize within timeout period',
            details: error
          });
          reject(error);
        }
  ...
  private scheduleReconnect(): void {
    ...
    console.log(`Scheduling Cast SDK reconnect attempt ${this.reconnectAttempts}/${this.maxReconnectAttempts} in ${delay}ms`);
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        if (this.currentSession.state !== 'connected') {
          reject(new Error(`Cast session not ready: ${this.currentSession.state}`));
          return;
        }
    ...
    try {
      ...
      receiverMediaInfo.customData = {
        originalMedia: {
          ...
          videoId: this.isYouTubeUrl(mediaInfo.contentId) ? this.extractYouTubeVideoId(mediaInfo.contentId) : null
        }
      };
  ...
  private loadStandardMedia(
    ...
    try {
      ...
      castMediaInfo.streamType = mediaInfo.streamType === 'LIVE' 
  ...
  private _sendCustomReceiverDemo(mediaInfo: MediaInfo): void {
    console.log('📺 Custom Receiver Demo - Would send to receiver:', {
      action: 'loadYouTubeVideo',
      videoId: this.extractYouTubeVideoId(mediaInfo.contentId),
      metadata: mediaInfo.metadata,
      receiverUrl: CUSTOM_RECEIVER_URL
    });
    
    // In a real custom receiver setup, we would send custom messages like:
    // session.sendMessage('urn:x-cast:vibez.media', { 
    //   action: 'loadYouTube', 
    //   videoId: videoId,
    //   metadata: mediaInfo.metadata 
    // });
  ...
  async updateQueue(queue: any[]): Promise<void> {
    try {
      ...
      session.sendMessage('urn:x-cast:vibez.queue', queueMessage, 
  ...
  async updateRoomInfo(roomInfo: { name: string; participantCount: number }): Promise<void> {
    try {
      if (!this.currentSession || this.currentSession.state !== 'connected') {
        return; // Silently return if no active session
      }
      ...
      session.sendMessage('urn:x-cast:vibez.room', roomInfo, 
  ...
  async syncPlaybackState(state: any): Promise<void> {
    try {
      ...
      if (state.positionMs && typeof state.positionMs === 'number') {
        ...
        if (timeDifference > 2000) { // 2 second threshold
  ...
  destroy(): void {
    try {
      ...
      if (this.currentSession) {
        this.disconnectFromDevice(this.currentSession.deviceId).catch(error => {
